...
async function pMy(openId) {
  ...
  try {
    ...
    view().innerHTML = '<div class="card"><h3>All interviews</h3><p class="sub">All saved interviews from every account — open, view and print. Edit and delete are limited to the interview owner and administrators.</p><div class="toolbar"><input id="q" placeholder="Search interviews…"></div><div id="list" class="grid g2"></div></div>';
    const draw = () => {
      const s = $('#q').value.toLowerCase();
      const item = (i) => {
        const mine = S.user && i.userId === S.user.id;
        ...
        let html = '<div class="qa-card"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><b style="flex:1;min-width:150px">' + esc(i.title) + '</b>' + badge(i.status) + '</div>'
          + '<p style="color:var(--muted);font-size:13px;margin:8px 0">' + (i.ownerName ? '👤 ' + esc(i.ownerName) + (mine ? ' (you)' : '') + ' · ' : '') + esc(i.stakeholders) + ' · ' + esc(i.date) + ' · ' + esc(i.interviewMethod || '—') + ' · ' + esc(i.interviewFormat || '—') + ' · ' + i.answerCount + '/' + i.questionCount + ' answered</p>'
      ...
      $$('#list [data-a]').forEach(b => { const it = myCache.find(x => x.id === b.dataset.id) || {}; b.onclick = () => myAction(b.dataset.a, b.dataset.id, isAdmin() || (S.user && it.userId === S.user.id)); });
    };
    $('#q').oninput = draw; draw();
  } catch (e) { view().innerHTML = '<div class="alert alert-error">' + esc(e.message) + '</div>'; }
}
...
async function pResults(selId) {
  ...
  try {
    ...
    const allQ = (isAdmin() && S.scope === 'mine') ? '?scope=mine' : '';
    ...
    view().innerHTML = '<div class="card no-print"><div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">'
      ...
      + '<select id="rSel" style="flex:1;min-width:200px;border:1.5px solid var(--line);border-radius:10px;padding:9px 12px;background:var(--card);color:var(--ink)">' + all.map(i => '<option value="' + i.id + '"' + (i.id === id ? ' selected' : '') + '>' + esc((i.ownerName && (!S.user || i.userId !== S.user.id) ? '[' + i.ownerName + '] ' : '') + i.title) + ' (' + i.answerCount + '/' + i.questionCount + ')</option>').join('') + '</select>'
...
async function pHistory() {
  ...
  try {
    ...
    const draw = async () => {
      ...
      const actions = (i) => {
        const canManage = isAdmin() || (S.user && i.userId === S.user.id);
      ...
      $('#hList').innerHTML = j.interviews.length ? '<div class="table-wrap"><table><tr><th>Title</th><th>Owner</th><th>Method</th><th>Format</th><th>Stakeholders</th><th>Date</th><th>Status</th><th>Q&A</th><th>Actions</th></tr>' + j.interviews.map(i =>
        '<tr><td><b>' + esc(i.title) + '</b></td><td>' + (i.ownerName ? '👤 ' + esc(i.ownerName) : '—') + '</td><td>' + esc(i.interviewMethod || '—') + '</td><td>' + esc(i.interviewFormat || '—') + '</td><td><small>' + esc(i.stakeholders) + '</small></td><td>' + esc(i.date) + '</td><td>' + badge(i.status) + '</td><td>' + i.answerCount + '/' + i.questionCount + '</td>'
      ...
      $$('#hList [data-a]').forEach(b => b.onclick = () => {
        ...
        else if (a === 'edit') { const it = all.find(x => x.id === id) || {}; if (isAdmin() || (S.user && it.userId === S.user.id)) editInterview(id, () => pHistory()); else toast('Only the interview owner or an administrator can edit this.', 'warning'); }
        else if (a === 'del') { const it = all.find(x => x.id === id) || {}; if (isAdmin() || (S.user && it.userId === S.user.id)) confirmDlg('Delete interview?', 'Are you sure you want to delete this interview?', 'Delete', async () => { await api('/api/interviews/' + id, { method: 'DELETE' }); toast('Interview deleted successfully.'); pHistory(); }); else toast('Only the interview owner or an administrator can delete this.', 'warning'); }
      });
    };
...
async function pReport(selId, scope) {
  ...
  try {
    ...
    view().innerHTML = '<div class="card no-print"><div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">'
      ...
      + '<select id="sSel" style="flex:1;min-width:200px;border:1.5px solid var(--line);border-radius:10px;padding:9px 12px;background:var(--card);color:var(--ink)">' + all.map(i => '<option value="' + i.id + '"' + (i.id === id ? ' selected' : '') + '>' + esc((i.ownerName && (!S.user || i.userId !== S.user.id) ? '[' + i.ownerName + '] ' : '') + i.title) + '</option>').join('') + '</select>'
      ...
      + (isAdmin() || (S.user && iv && iv.userId === S.user.id) ? '<button class="btn btn-ghost btn-sm" id="sEdit">Edit</button><button class="btn btn-danger btn-sm" id="sDel">Delete</button>' : '')
    ...
    $('#sEdit').onclick = () => { if (isAdmin() || (S.user && iv.userId === S.user.id)) editInterview(id, () => pReport(id)); else toast('Only the interview owner or an administrator can edit this.', 'warning'); };
    $('#sDel').onclick = () => { if (isAdmin() || (S.user && iv.userId === S.user.id)) confirmDlg('Delete interview?', 'Are you sure you want to delete this interview?', 'Delete', async () => { await api('/api/interviews/' + id, { method: 'DELETE' }); toast('Interview deleted successfully.'); go('history'); }); else toast('Only the interview owner or an administrator can delete this.', 'warning'); };
    ...
    $('#sumCards').innerHTML = all.map(c => {
      ...
      return '<div class="sum-card"><div class="sum-top ' + cls + '"><div style="display:flex;gap:10px;align-items:center">' + (c.ownerAvatar ? '<img class="sum-avatar" src="' + esc(c.ownerAvatar) + '" alt="avatar">' : '') + '<div style="flex:1;min-width:0"><b>' + esc(c.title) + '</b><small>' + esc(c.interviewee || c.stakeholders.split(',')[0] || '—') + ' · ' + badge(c.status) + '</small></div></div></div>'
        ...
        + '</div><div class="sum-foot no-print"><button class="btn btn-primary btn-sm" data-v="' + c.id + '">Open</button><button class="btn btn-ghost btn-sm" data-p="' + c.id + '">🖨 Print</button>' + ((isAdmin() || (S.user && c.userId === S.user.id)) ? '<button class="btn btn-danger btn-sm" data-d="' + c.id + '">Delete</button>' : '') + '</div></div>';
    }).join('');
