...
/* Shared system: EVERY logged-in account can VIEW all interviews & summary reports.
   Editing/deleting/generating is limited to the interview owner or an Administrator. */
const canManage = (req, iv) => iv && (req.user.role === 'Administrator' || iv.userId === req.user.id);
const getIvAny = (id) => db.getInterview(null, id);
/* administrator scope: ?scope=mine shows only the admin's own interviews; default shows all */
const scopeUid = (req) => (String(req.query.scope || '') === 'mine') ? req.user.id : null;
...
app.get('/api/recent', requireAuth, (req, res) => res.json({ ok: true, interviews: db.listInterviews(null).map(db.withCounts).slice(0, 6) }));
...
app.get('/api/interviews/:id', requireAuth, (req, res) => {
  ...
  res.json({ ok: true, interview: db.withCounts(iv), suggestion: db.getSuggestion(null, iv.id) });
});
app.put('/api/interviews/:id', requireAuth, (req, res) => {
  const existing = db.getInterview(null, req.params.id);
  if (!existing) return res.status(404).json({ ok: false, error: 'Interview not found.' });
  if (!canManage(req, existing)) return res.status(403).json({ ok: false, error: 'Only the interview owner or an administrator can edit this interview.' });
  const b = req.body || {};
  const patch = {};
...
app.delete('/api/interviews/:id', requireAuth, (req, res) => {
  const existing = db.getInterview(null, req.params.id);
  if (!existing) return res.status(404).json({ ok: false, error: 'Interview not found.' });
  if (!canManage(req, existing)) return res.status(403).json({ ok: false, error: 'Only the interview owner or an administrator can delete this interview.' });
  if (!db.deleteInterview(null, req.params.id)) return res.status(404).json({ ok: false, error: 'Interview not found.' });
  res.json({ ok: true });
});
...
app.get('/api/interviews/:id/questions', requireAuth, (req, res) => {
  const rows = db.listQuestions(null, req.params.id);
...
app.put('/api/questions/:qid', requireAuth, (req, res) => {
  const q0 = db.getQuestion(req.params.qid);
  const iv0 = q0 && db.getInterview(null, q0.interviewId);
  if (!iv0) return res.status(404).json({ ok: false, error: 'Question not found.' });
  if (!canManage(req, iv0)) return res.status(403).json({ ok: false, error: 'Only the interview owner or an administrator can edit this question.' });
  ...
  const q = db.updateQuestion(null, req.params.qid, patch);
...
app.delete('/api/questions/:qid', requireAuth, (req, res) => {
  ...
  const iv0 = q0 && db.getInterview(null, q0.interviewId);
  if (!iv0) return res.status(404).json({ ok: false, error: 'Question not found.' });
  if (!canManage(req, iv0)) return res.status(403).json({ ok: false, error: 'Only the interview owner or an administrator can delete this question.' });
  if (!db.deleteQuestion(null, req.params.qid)) return res.status(404).json({ ok: false, error: 'Question not found.' });
  res.json({ ok: true });
});
app.post('/api/questions/:qid/answer', requireAuth, (req, res) => {
  const r = db.saveAnswer(null, req.params.qid, clean((req.body || {}).answer, 8000));
...
app.post('/api/interviews/:id/next', requireAuth, async (req, res) => {
  try {
    ...
    const iv = db.getInterview(null, req.params.id);
    if (!iv) return res.status(404).json({ ok: false, error: 'Interview not found.' });
    if (!canManage(req, iv)) return res.status(403).json({ ok: false, error: 'Only the interview owner or an administrator can run the interview flow for this interview.' });
    const qa = db.listQuestions(null, iv.id) || [];
    ...
    if (method === 'Semi-Structured') {
      ...
      const saved = db.addQuestion(null, iv.id, { text: out.text, category: out.category || 'Follow-up', stakeholder: lastAns.stakeholder, isFollowUp: 1, parentNumber: lastAns.number });
    ...
    const saved = db.addQuestion(null, iv.id, { text: out.text, category: out.category || 'Adaptive', stakeholder: (lastAns && lastAns.stakeholder) || (iv.stakeholders || '').split(',')[0].trim() || 'General', isFollowUp: answered.length ? 1 : 0, parentNumber: lastAns ? lastAns.number : null });
...
app.post('/api/interviews/:id/analyze', requireAuth, async (req, res) => {
  try {
    const iv = db.getInterview(null, req.params.id);
    if (!iv) return res.status(404).json({ ok: false, error: 'Interview not found.' });
    if (!canManage(req, iv)) return res.status(403).json({ ok: false, error: 'Only the interview owner or an administrator can generate the report for this interview.' });
    const qa = db.listQuestions(null, iv.id);
    ...
    const saved = db.saveSuggestion(null, iv.id, out.analysis, out.source);
...
app.get('/api/interviews/:id/suggestion', requireAuth, (req, res) => {
  const s = db.getSuggestion(null, req.params.id);
